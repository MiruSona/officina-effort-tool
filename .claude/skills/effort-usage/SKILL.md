...
> **`estimate` 값은 사람 시간이 아니다.** 본줄만 세니 `구현:L` 이 4분쯤으로 나온다.
> 사람이 앉아 있던 시간은 그보다 몇 배 크다. 소단계 표의 **예상 칸에 그대로 옮겨 적지 말고**,
> 소단계끼리의 **순서와 크기 차이**를 보는 데 쓴다. 사람 시간 눈금은 `actual` 로 회차를
> 15~20건 쌓아 `human` 배율을 잡은 뒤에야 나온다.
